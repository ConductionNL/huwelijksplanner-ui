...
class RequestService
{
    ...
    public function createFromRequestType($requestType, $requestParent = null ,$user = null, $organization= null, $application= null)
    {
    	...
    	$request = $this->commonGroundService->createResource($request, 'https://vrc.huwelijksplanner.online/requests'); // HP specifiek
    	...
    	if($requestParent){
    		...
    		// Lets transfer any properties that are both in the parent and the child request
    		foreach($requestType['properties'] as $property){
                $slug = $property['slug'];
                
                // Moeten we nog een keer naar kijken
                if($slug == 'getuige'){
                        $slug = 'getuigen';
                }
                elseif($slug == 'partner'){
    	...
    	$contact= $this->commonGroundService->createResource($contact, $this->commonGroundService->getComponent('cc').'/people'); // HP specifiek
        ...
        if(!key_exists('partners', $request)){
            ...
            $assent = $this->commonGroundService->createResource($assent, $this->commonGroundService->getComponent('irc').'/assents'); // HP specifiek 
