...
class DefaultController extends AbstractController
{
    ...
    public function submitrequestAction(Session $session, CommonGroundService $commonGroundService, MessageService $messageService, RequestService $requestService)
    {
        ...
        if ($request = $requestService->updateRequest($request, $request['@id'])) {
            ...
            if (array_key_exists('emails', $contact) && array_key_exists(0, $contact['emails'])) {
...
//                $session->set('request', $request);
     ...
     */
    public function newRequest(Session $session, $slug, $value, ApplicationService $applicationService, RequestService $requestService, CommonGroundService $commonGroundService, Request $request)
    {
    }
...
//            die;
        ...
        if (is_array($variables['request']['properties'])) {
            ...
            elseif ($value == 'https://pdc.huwelijksplanner.online/offers/2b9ba0a9-376d-45e2-aa83-809ef07fa104' || $value == 'https://pdc.dev.huwelijksplanner.online/offers/2b9ba0a9-376d-45e2-aa83-809ef07fa104') {
                ...
                if (array_key_exists('locatie', $variables['request']['properties']) && $slug == 'plechtigheid') {
                    unset($variables['request']['properties']['locatie']);
                    $this->addFlash('success', 'U kunt nu een locatie kiezen');
                }
                if (array_key_exists('ambtenaar', $variables['request']['properties']) && $slug == 'plechtigheid') {
        ...
        unset($variables['request']['submitters']);
        if (array_key_exists('parent', $variables['request'])) {
            unset($variables['request']['parent']);
        }
        if (array_key_exists('children', $variables['request'])) {
            unset($variables['request']['children']);
        }
        if ($variables['request'] = $requestService->updateRequest($variables['request'], $variables['request']['@id'])) {
            $session->set('request', $variables['request']);
     ...
     */
    public function paymentAction(Session $session, $slug, $resource, SjabloonService $sjabloonService, Request $httpRequest, CommonGroundService $commonGroundService, ApplicationService $applicationService, RequestService $requestService)
    {
        $variables = $applicationService->getVariables();
        if (!array_key_exists('order', $variables['request']['properties'])) {
        ...
        $order['url'] = $order['@id'];
        if (array_key_exists('invoice', $variables['request']['properties']) && $variables['request']['properties']['invoice'] != null) {
     ...
     */
    public function payedAction(Session $session, $id, $slug, $resource, SjabloonService $sjabloonService, Request $httpRequest, CommonGroundService $commonGroundService, ApplicationService $applicationService, RequestService $requestService)
    {
        if (!$id) {
     ...
     */
    public function viewAction(Session $session, $slug, $resource, SjabloonService $sjabloonService, Request $httpRequest, CommonGroundService $commonGroundService, ApplicationService $applicationService, RequestService $requestService)
    {
        $variables = $applicationService->getVariables();
